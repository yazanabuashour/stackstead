...
4. Removes a Stackstead-owned Git worktree, or only the generated `.stackstead` directory for externally owned source. If normal removal of Stackstead-owned source fails, it uses a pinned minimal container with write access only to that exact worktree to restore host ownership and permissions, then retries. A read-only mount of the manifest's state directory provides the owner as seen inside the helper's user namespace, so rootless Docker does not misinterpret host user and group IDs. The helper verifies the runtime-token claim; external source and ordinary removable worktrees never invoke it.
